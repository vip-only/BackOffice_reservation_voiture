...
public class PlanificationReservationController {
    ...
    public ModelView afficherPlanification(@RequestParam("date") String dateStr) {
        ...
        try {
            ...
            List<Reservation> reservations = r.getReservationAssignes();
            System.out.println("taille des reservations assignées : " + reservations.size());
            // r.assignerVehicules(reservations);
            List<Vehicule> v = r.getPlusPetiteCapacite(reservations);
            System.out.println("Véhicules assignés : " + v.size());
            for (Vehicule vehicule : v) {
                int nb = vehicule.getNombrePlace()-vehicule.getNbPlaceReservees();
                System.out.println(vehicule.getReference() + "-" + nb); 
            }
            ...
            mv.addData("reservationsSansVehicule", reservations);
